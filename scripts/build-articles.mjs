...
function writeArticleRoutes() {
  const routes = {}
  for (const [groupId, group] of GROUPS) {
    routes[groupId] = {}
    for (const [lang, article] of Object.entries(group)) {
      routes[groupId][lang] = article.slug
    }
  }
  const js = `// Auto-generated by scripts/build-articles.mjs — do not edit
export const ARTICLE_ROUTES = ${JSON.stringify(routes, null, 2)}
...
`
  writeFileSync(join(root, 'src', 'article-routes.js'), js)
  console.log(`Wrote article routes (${Object.keys(routes).length} groups) → src/article-routes.js`)
}
